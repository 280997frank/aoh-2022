...
export const sendEmailDaily = async () => {
  try {
    ...
    if (thisday.isBefore(lastday)) {
      ...
      await generalQuery.then((documents) => {
        documents.docs.map((docs) => {
          ...
          if (
            docs.data()?.event === "click" &&
            docs.data()?.type === "redeem"
          ) {
      ...
      // .limit(5);
      const filename = `daily_report_aoh22sg_${today}.xlsx`;
      ...
      const send = await transporter.sendMail({
        ...
        // cc: ["zodiacsolutionspl@gmail.com","xiangsheng.chee@gmail.com","jojo.lim@pico.com"],
        to: "martin.mamanua@gmail.com",
        subject: "AOH22 SG Daily Report",
        html: "Daily Report",
        text: "Daily Report",
        attachments: [
          {
